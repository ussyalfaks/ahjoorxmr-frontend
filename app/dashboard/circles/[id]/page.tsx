...
export default function CircleDetailPage({
  ...
  const health = calculateCircleHealth({
    onTimeContributions: circle.participants.filter((participant) => participant.paid).length,
    expectedContributions: circle.participants.length,
    completedPayouts: circle.roundHistory.length,
    expectedPayouts: Math.max(circle.currentRound - 1, circle.roundHistory.length),
